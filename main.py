...
def get_select_level():
  ...
  difficult_level = input("Type your difficult level: ").lower(
  )  # Zamieniamy na małe litery dla spójności
  ...
  if difficult_level in levels:
    ...
    number_of_lives = levels[
        difficult_level]  # Pobranie ilości żyć na podstawie klucza
  else:
    print("Chosen difficulty level not found, defaulting to easy.")
    number_of_lives = levels[
        "easy"]  # Domyślnie ustawiamy ilość żyć na poziomie łatwym
  ...
  while number_of_lives > 0 and num_of_letters > 0:
    ...
    for index, char in enumerate(tab_of_letters):
      if char == guess:
        ...
        print(
            f'You guessed {char}, which has {index} index, you have {num_of_letters} letters  and  {number_of_lives} lifes left'
        )
...
input_string = getpass.getpass("Wprowadź tekst: ")
